...
let Cookie = function () {
    ...
    var config = Cookie.cookie = function (key, value, options) {
        ...
        if (arguments.length > 1) {
            options = Tool.assign({}, config.defaults, options);
    ...
    Cookie.removeCookie = function (key, options) {
        ...
        Cookie.cookie(key, '', Tool.assign({}, options, { expires: -1 }));
    ...
    Cookie.clearCookie = function () {
        ...
        for (var i = 0; i < aCookie.length; i++) {
            ...
            document.cookie = aCrumb[0] + "=" + aCrumb[1] + ";path=/;expires=" + exp.toUTCString();
        }
    };
    return Cookie;
};

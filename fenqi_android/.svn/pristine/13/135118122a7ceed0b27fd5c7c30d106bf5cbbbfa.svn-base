...
public class ComposerLayout extends RelativeLayout {
	...
	public RelativeLayout rlButton;// 主按鈕
	...
	private int align1 = 12, align2 = 14;
	 ...
	 *            動畫耗時
	 */
	public void init(int[] imgResId,final int showhideButtonId,final int hideButtonId, int crossId,
			byte pCode, int radius, final int durationMillis,OnComposerLayoutChangeListener listener) {
		...
		for (int i = 0; i < imgResId.length; i++) {
			...
			LinearLayout.LayoutParams llps = new LinearLayout.LayoutParams(RelativeLayout.LayoutParams.WRAP_CONTENT ,RelativeLayout.LayoutParams.WRAP_CONTENT);

			img.setLayoutParams(llps);
			llayouts[i] = new LinearLayout(mycontext);// 子按鈕層
			llayouts[i].setId(imgResId[i]);// 隨便設個id，方便onclick嘅時候識別返出嚟。呢個id值係求其設嘅，如果發現同其他控件沖突就自行改一下。
		...
		rlButton.setBackgroundResource(hideButtonId);

    ...
    render: function () {
                            ...
                            <p>
                                {
                                    item[this.props.subTypeName].map(function (subItem, subIndex) {
                                        return (<a key={subIndex}
                                                   onClick={this.onChange(index,subItem[this.props.subItemName],subItem[this.props.subItemId])}><span>{subItem[this.props.subItemName]}</span></a>)
                                    }.bind(this))
                                }
                            </p>
                        </div>
        ...
        return (
            <div className="ft-logo">
                <div
                    style={{float: 'left',lineHeight: '35px',color: '#999',paddingRight: '30px'}}>{this.props.kindName}</div>
                <div className="filterCont">
                    ...
                    cursor: 'pointer',
                    color: 'rgb(233,113,125)'
                } : {display: 'inline-block', paddingRight: '12px', paddingBottom: '12px', cursor: 'pointer'}}
                                                    onClick={this.onChange(-1,'不限')}>不限</em></span>
                </div>
                {nodes}
                <div style={{clear:'both'}}></div>
            </div>
...
var R_ProjectListPage = React.createClass({
    ...
    queryGoodsList() {
        ...
        $.ajax({
            ...
            success: (res)=> {
                if (res.result == 0) {
                    this.setState({
                        list: res.data.goodsItemList,
                        areas: res.data.provinceVOList,
                        kinds: res.data.categoryList,
                        index: res.data.paginator.totalPage
                    })
                }
                console.log(res);
            }
    ...
    componentWillMount() {
        ...
        if (this.getUrlParam('search') != null) {
            var search = this.getUrlParam('search');
            console.log(search);
            this.onFilterChange('search', '搜索条件：' + search);
            this.setQueryCondition('search', 'searchInfo', search);
        }
        this.queryGoodsList();
    },
        ...
        return (
            <div style={this.styles.container}>
                <div style={this.styles.filter}>
                    <DoubleFilter kindName={'地区'} typeName={'proname'} subTypeName={'cityList'} subItemName={'cityname'}
                                  cxt={this}
                                  items={!!this.state.areas ? this.state.areas : []} subItemId={'cityid'}
                                  callbackId={'cityId'}/>
                    <DoubleFilter kindName={'类别'} typeName={'categoryName'} subTypeName={'categoryList'}
                                  subItemName={'categoryName'} cxt={this}
                                  items={!!this.state.kinds ? this.state.kinds : []} subItemId={'categoryId'}
                                  callbackId={'categoryId'}/>
                ...
                <div style={{marginTop: '12px', marginBottom: '12px'}}>
                    <Pagination cxt={this} numberOfPages={this.state.index}/>
                </div>
            </div>
        )
    }
